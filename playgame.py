...
def main():
    print('Welcome to Connect 3 by Matthew Hillery')
    mode = get_mode()
    while mode != 'Q':
        if mode == 'A':
            run_two_player_mode()
...
def run_two_player_mode():
    ...
    rows = get_int("Please select number of rows (Min: {}  Max: {}): ".format(MIN_ROWS, MAX_ROWS), MIN_ROWS,
                           MAX_ROWS)
    cols = get_int("Please select number of columns (Min: {}  Max: {}): ".format(MIN_COLS, MAX_COLS), MIN_COLS,
                           MAX_COLS)
    ...
    while game.get_winner() is None:
        move = get_int("Player {}'s turn. Choose column (0 to {})".format(game.get_whose_turn(), cols - 1), 0,
                               cols - 1)
...
def run_ai_mode():
    ...
    while game.get_winner() is None:
        if game.get_whose_turn() == player:
            move = get_int("Your turn. Choose column (0 to {}): ".format(cols - 1), 0, cols - 1)
    ...
    while True:
        try:
            ...
            if low <= num <= high:
                ...
                print("Value must be between {} and {}: ".format(low, high))
        except ValueError:
            print("Value must be between {} and {}: ".format(low, high))
